...
import '../cubits/errors_cubit.dart';
import '../cubits/errors_state.dart';
...
class ErrorsTabContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ErrorsCubit, ErrorsState>(
      builder: (context, state) {
        final headerWidgets = [
          PostagePrivacyBanner(text: context.l10n.errorBoxPrivacyBanner),
          _AutoSendToggle(),
        ];
          ...
          content: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ErrorCard(
                ...
                onDelete: () => context.read<ErrorsCubit>().deleteError(error.id),
              );
            },
          ),
  ...
  Future<void> _sendAndPromptClear(BuildContext context, String errorId) async {
    final cubit = context.read<ErrorsCubit>();
    ...
    if (state.status == UiFlowStatus.success &&
        state.lastOperation == ErrorsOperation.sendOne) {
          ...
          child: Row(
            children: [
              ...
              QuanityaToggle(
                ...
                onChanged: (value) =>
                    context.read<ErrorsCubit>().toggleAutoSend(value),
              ),
            ],
          ),
        );
...
class _BottomActions extends StatelessWidget {
  ...
  Future<void> _sendAndPromptClear(BuildContext context) async {
    ...
    final state = cubit.state;
    if (state.status == UiFlowStatus.success &&
        state.lastOperation == ErrorsOperation.sendAll &&
