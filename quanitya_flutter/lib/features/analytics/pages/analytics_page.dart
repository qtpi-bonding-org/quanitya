          ...
          child: Row(
            children: [
              ...
              QuanityaToggle(
                ...
                onChanged: (value) =>
                    context.read<AnalyticsCubit>().toggleAutoSend(value),
              ),
            ],
          ),
        );
    ...
    return BlocBuilder<AnalyticsCubit, AnalyticsState>(
      builder: (context, state) {
        final headerWidgets = <Widget>[
          PostagePrivacyBanner(text: context.l10n.analyticsInboxPrivacyNotice),
          _AutoSendToggle(),
        ];
...
class _BottomActions extends StatelessWidget {
  ...
  Future<void> _sendAndPromptClear(BuildContext context) async {
    ...
    if (state.status == UiFlowStatus.success &&
        state.lastOperation == AnalyticsOperation.sendAll &&
  ...
  void _confirmClearAll(BuildContext context) {
    QuanityaConfirmationDialog.show(
      ...
      isDestructive: true,
      onConfirm: () => context.read<AnalyticsCubit>().clearAll(),
    );
  }
}
